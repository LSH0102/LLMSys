...
def generate(
    ...
    for example in tqdm.tqdm(examples, desc=f'Generating {desc}'):
        ...
        while len(token_ids) <= model_max_length:
            ...
            gen_id = 0
            input_id=minitorch.tensor_from_numpy(np.array(token_ids),backend=backend).view(1,len(token_ids))
            
            logits=model(input_id).to_numpy()
            prob=logits[0,-1,:]
            gen_id=np.argmax(prob,axis=-1)
