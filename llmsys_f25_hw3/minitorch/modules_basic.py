...
class Embedding(Module):
    def __init__(self, num_embeddings: int, embedding_dim: int, backend: TensorBackend):
        ...
        ### BEGIN ASSIGN3_2
        self.weights=Parameter(tensor_from_numpy(np.random.normal(0,1,size=(self.num_embeddings,self.embedding_dim)),backend=self.backend))
        ### END ASSIGN3_2
    ...
    def forward(self, x: Tensor):
        ...
        bs, seq_len = x.shape
        ### BEGIN ASSIGN3_2
        index=one_hot(x, num_classes=self.num_embeddings)
        index=index.view(*(bs*seq_len,self.num_embeddings))
        y=index.__matmul__(self.weights.value)
        y=y.view(*(bs,seq_len,self.embedding_dim))
        return y
        ### END ASSIGN3_2
...
class Dropout(Module):
    ...
    def forward(self, x: Tensor) -> Tensor: 
        ...
        return mask*x/(1.0-self.p_dropout)
        ### END ASSIGN3_2
...
class Linear(Module):
    def __init__(self, in_size: int, out_size: int, bias: bool, backend: TensorBackend):
        ...
        ### BEGIN ASSIGN3_2
        self.weights=Parameter(tensor_from_numpy(np.random.uniform(-1/np.sqrt(in_size),1/np.sqrt(in_size),
                                                                   size=(in_size,out_size)),backend=backend))
    
        self.bias=Parameter(tensor_from_numpy(np.random.uniform(-1/np.sqrt(in_size),1/np.sqrt(in_size),size=(out_size,)),backend=backend))
    ...
    def forward(self, x: Tensor):
        ...
        ### BEGIN ASSIGN3_2
        if self.apply_bias==False:
            output=x.__matmul__(self.weights.value)
        else:
            
            output=x.__matmul__(self.weights.value)+self.bias.value
        return output
        ### END ASSIGN3_2
...
class LayerNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        var=x.var(dim=1)
        mean=x.mean(dim=1)
        x_hat=(x-mean)/(var+self.eps).__pow__(0.5)
