...
__global__ void mapKernel(
   ...
   */
    ...
    int thread_offset=threadIdx.y+threadIdx.x*blockDim.y;

    int position=block_offset+thread_offset;
...
__global__ void reduceKernel(
   ...
   */
    ...
    to_index(position,  out_shape, out_index, shape_size);
    ///因为是规约运算 所以out_index肯定对应a_storage中的pos_a
    int pos_a=index_to_position(out_index, a_strides, shape_size);
    int i;
    int step=a_strides[reduce_dim];
    int a_size=out_size*a_shape[reduce_dim];
    out[position]=reduce_value;
    
    if(pos_a<0)
    {
        return;
    }
    for(i=0;i<a_shape[reduce_dim];i++)
    {   
        out[position]=fn(fn_id,a_storage[pos_a+i*step],out[position]);
...
__global__ void MatrixMultiplyKernel(
   ...
   */
    ...
    int out_size=out_shape[0]*out_shape[1]*out_shape[2];
    ...
    for(i=0;i<n;i++)
    {
        c+=a_storage[batch*a_batch_stride+row*a_strides[1]+i*a_strides[2]]*b_storage[batch*b_batch_stride+i*b_strides[1]+col*b_strides[2]];
