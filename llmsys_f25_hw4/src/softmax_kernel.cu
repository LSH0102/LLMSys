...
__global__ void ker_attn_softmax_bw(T *grad, const T *inp, int softmax_length) {
  ...
  for (int i = 0; i < ITERATIONS; ++i) {
    ...
    if (curr_idx < softmax_length) {
      ...
      //printf("%f %f %d %d\n",grad_reg[i],inp_reg[i],threadIdx.x,threadIdx.y);
    }
  }
...
void launch_attn_softmax_bw(float *out_grad,
  ...
  int input_size=(rows*softmax_len)*sizeof(float);
  float *d_grad, *d_soft;
  cudaMalloc((void **)&d_grad, input_size);
  cudaMalloc((void **)&d_soft, input_size);
  
  cudaMemcpy(d_grad, out_grad, input_size, cudaMemcpyHostToDevice);
  cudaMemcpy(d_soft, soft_inp, input_size, cudaMemcpyHostToDevice);
 
  
  int it=input_size/WARP_SIZE;
  
  if(softmax_len<=WARP_SIZE){
    ker_attn_softmax_bw<float, 1><<<grid_dim, block_dim,0, stream>>>(d_grad,d_soft,softmax_len);
  } else if(softmax_len<=2*WARP_SIZE){
    ker_attn_softmax_bw<float, 2><<<grid_dim, block_dim,0, stream>>>(d_grad,d_soft,softmax_len);
  }else if(softmax_len<=4*WARP_SIZE){
    ker_attn_softmax_bw<float, 4><<<grid_dim, block_dim,0, stream>>>(d_grad,d_soft,softmax_len);
  }else if(softmax_len<=8*WARP_SIZE){
    ker_attn_softmax_bw<float, 8><<<grid_dim, block_dim,0, stream>>>(d_grad,d_soft,softmax_len);
  }else if(softmax_len<=16*WARP_SIZE){
    ker_attn_softmax_bw<float,16><<<grid_dim, block_dim,0, stream>>>(d_grad,d_soft,softmax_len);
  }else if(softmax_len<=32*WARP_SIZE){
  ...
  // Copy back to the host
  cudaMemcpy(out_grad, d_grad, input_size, cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
