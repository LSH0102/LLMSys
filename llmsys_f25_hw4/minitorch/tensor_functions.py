...
class Attn_Softmax(Function):
    ...
    def forward(ctx: Context, inp: Tensor, mask: Tensor) -> Tensor:
      #   BEGIN ASSIGN4_1_1
      soft=inp.f.attn_softmax_fw(inp,mask)
      ctx.save_for_backward(soft,mask)
      return soft
      #   END ASSIGN4_1_1
...
class LayerNorm(Function):
    ...
    def forward(ctx: Context, inp: Tensor, gamma: Tensor, beta: Tensor) -> Tensor:
      #   BEGIN ASSIGN4_2_1
      res,mean,std=inp.f.layernorm_fw(inp,gamma,beta)
      ctx.save_for_backward(inp,gamma,beta,mean,std)
      return res
      #   END ASSIGN4_2_1
    ...
    def backward(ctx: Context, out_grad: Tensor) -> Tensor:
      #   BEGIN ASSIGN4_2_2
      inp,gamma,beta,mean,std=ctx.saved_values
      return out_grad.f.layernorm_bw(out_grad,inp,gamma,beta,std,mean)
