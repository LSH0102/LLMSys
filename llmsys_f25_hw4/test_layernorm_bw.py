...
backend1 = minitorch.TensorBackend(SimpleOps)
...
def test_launch_layernorm_bw():
    ...
    def custom():
      ...
      inp_grad = torch.tensor(inp_mt.grad.to_numpy(), dtype=torch.float32)
      gamma_grad = torch.tensor(gamma_mt.grad.to_numpy(), dtype=torch.float32)
      betta_grad = torch.tensor(beta_mt.grad.to_numpy(), dtype=torch.float32)
    ...
    def baseline():
      ...
      inp_grad = torch.tensor(dinp.to_numpy(), dtype=torch.float32)
      gamma_grad = torch.tensor(f_gamma_grad.to_numpy(), dtype=torch.float32)
      betta_grad = torch.tensor(f_betta_grad.to_numpy(), dtype=torch.float32)
