...
def cross_entropy_loss(out, y):
    ...
    L=-y*out.log()-(ones-y)*(ones-out).log()
    L=L.mean()
    return L
    # END ASSIGN1_3
...
class Linear(minitorch.Module):
    ...
    def forward(self, x):
        ...
        #x=minitorch.tensor_functions.View.apply(x,(batch, in_size))
        #w=minitorch.tensor_functions.View.apply(self.weights,(in_size, self.out_size))
        
        out=minitorch.tensor_functions.MatMul.apply(x,self.weights.value)+self.bias.value
        return out
...
class Network(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        out=embeddings.mean(dim=1)
        out=minitorch.tensor_functions.View.apply(out,minitorch.tensor([b,e]))
        out=self.Linear1(out)
        out=minitorch.tensor_functions.ReLU.apply(out)
        
        out=minitorch.dropout(out, rate=self.dropout_prob)
        
        out=self.Linear2(out)
        out=minitorch.tensor_functions.Sigmoid.apply(out)
        out=minitorch.tensor_functions.View.apply(out,minitorch.tensor([b]))
        return out
